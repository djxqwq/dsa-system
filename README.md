...
- **开启方式**：点击登录页面左上角品牌名称"驾校系统"，输入管理员账号验证后开启
- **功能特性**：
  - 一键管理员登录：无需输入账号密码，直接以管理员身份登录
  - 一键学员登录：快速选择任意学员账号登录
  - 一键教练登录：快速选择任意教练账号登录
- **使用场景**：开发测试、功能演示、快速切换角色验证
...
| ------------------- | ------------------------------------- |
| admin               | 管理员信息表                                |
| coach               | 教练信息表（含work\_status在岗状态字段）            |
| student             | 学员信息表（含gender性别字段）                    |
| vehicle             | 车辆信息表                                 |
| schedule            | 教练排班表                                 |
| appointment         | 预约记录表                                 |
| maintenance\_record | 保养记录表                                 |
| repair\_record      | 维修记录表                                 |
| system\_config      | 系统配置表                                 |
| message             | 消息表（含student\_id学员ID、coach\_id教练ID字段） |
...
| ------------------------ | ----- | ---------- |
| open\_days               | 7     | 开放预约天数     |
| daily\_limit             | 1     | 单人每日预约上限   |
| cancel\_deadline         | 2     | 取消截止时间（小时） |
| confirm\_timeout         | 30    | 确认超时时间（分钟） |
| system\_notice           | -     | 系统公告       |
| min\_appointment\_hours  | 1     | 最小预约时长     |
| max\_appointment\_hours  | 4     | 最大预约时长     |
| slot\_duration           | 30    | 时间槽时长（分钟）  |
| enable\_sms\_notify      | false | 启用短信通知     |
| enable\_email\_notify    | false | 启用邮件通知     |
| working\_start\_time     | 08:00 | 工作时间开始     |
| working\_end\_time       | 18:00 | 工作时间结束     |
| default\_required\_hours | 12    | 默认要求学时     |
